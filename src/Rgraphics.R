...
setwd('8. Data Visualisation-20210723/')
...
#pass from scientific to numerical notation
options(scipen=99)
##   `ggplot2' histogram example:
str(housing)
library(ggplot2)
bars <- ggplot(housing, aes(x = Home.Value, fill=region)) +
  geom_histogram() + ggtitle('Histogram of house value',subtitle = 'in thousands $') + xlab('Home Value') +
  ylab('Freuency')
bars
...
scatter <- ggplot(subset(housing, State %in% c("MA", "TX")),
       aes(x=Date,
           y=Home.Value,
           color=State))+
...
hp1992Q2 <- housing %>% filter(Date==1992.50)
str(housing)
...
ggplot(hp2001Q1,
  ...
  geom_point()
require(ggrepel)
#let combine them and add the name of the 
ggplot(hp2001Q1, (aes(Land.Value, Structure.Cost, col=State))) + geom_point() +
  geom_label_repel(label=hp2001Q1$State, max.overlaps = 100) 
...
library("ggrepel")
p1 <- ggplot(hp2001Q1, aes(x = log(Land.Value), y = Structure.Cost, col=State))

p1 + 
  geom_point() + 
  geom_text_repel(aes(label=State), size = 6) +
...
ggplot(housing.sum, aes(x=State, y=Home.Value, col=State)) + 
  geom_bar(stat="identity", show.legend = F,width = .5) +theme(legend.position="none")
...
(p5 <- p5 + geom_line(aes(color=State)) +
   facet_wrap(~State, ncol = 10)) + theme(legend.position="none") 
(p5 <- p5 + geom_line(aes(color=State)) +
    facet_wrap(~State, nrow = 5)) + theme(legend.position="none") 
...
##   - `theme_bw()'
##   - `theme_classc()'
p5 + theme_linedraw() + theme(legend.position="none") 
...
##   example:
p5 + theme_minimal() +
  theme(text = element_text(color = "turquoise")) + theme(legend.position="none") 
...
p5 + theme_new + theme(legend.position="none") 
...
# aggregate split data into subdataset
...
# we put data in sort of wide format
